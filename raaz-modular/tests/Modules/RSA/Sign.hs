...
testPKCSVectors :: [(ByteString,ByteString)] -- Message,Signature
testPKCSVectors = [(BS.pack m,BS.pack sig)]
  where
    m = [0xcd,0xc8,0x7d,0xa2,0x23,0xd7,0x86,0xdf,0x3b,0x45,0xe0,0xbb,0xbc,0x72
        ,0x13,0x26,0xd1,0xee,0x2a,0xf8,0x06,0xcc,0x31,0x54,0x75,0xcc,0x6f,0x0d
        ,0x9c,0x66,0xe1,0xb6,0x23,0x71,0xd4,0x5c,0xe2,0x39,0x2e,0x1a,0xc9,0x28
...
testPSSSign :: PrivateKey -> (ByteString,ByteString,ByteString) -> Assertion
testPSSSign privk (m,salt,sig) = do
  o <- rsassaPSSSign sha1rec salt privk m
  o @=? sig

testPSSVerify :: PublicKey -> (ByteString,ByteString,ByteString) -> Assertion
testPSSVerify pubk (m,_,sig) = do
  b <- rsassaPSSVerify sha1rec pubk m sig
  assert b

signPSS :: Test
signPSS = testGroup "Signing PSS" $ map with testPSSVectors
  where
    with t@(m,_,_) = testCase (shorten $ show m)  $ testPSSSign testPrivKey t

verifyPSS :: Test
verifyPSS = testGroup "Verifying PSS" $ map with testPSSVectors
  where
    with t@(m,_,_) = testCase (shorten $ show m)  $ testPSSVerify testPubKey t


testPKCSSign :: PrivateKey -> (ByteString,ByteString) -> Assertion
testPKCSSign privk (m,sig) = do
  let h = hash' sha1rec m
  o <- rsassaPKCS1v1_5Sign h privk
  o @=? sig

testPKCSVerify :: PublicKey -> (ByteString,ByteString) -> Assertion
testPKCSVerify pubk (m,sig) = do
  let h = hash' sha1rec m
  b <- rsassaPKCS1v1_5Verify h pubk sig
  assert b
