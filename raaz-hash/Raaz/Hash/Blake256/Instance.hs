  ...
  apply (HGadget (cellBlake, cellSalt, cellCounter)) n cptr = do
    ...
    where
      ...
      moveAndHash salt (cxt, counter, ptr) _ = do
        let nCounter = counter + cryptoCoerce sz
...
instance PaddableGadget (HGadget BLAKE256) where
	unsafeApplyLast g@(HGadget (cellBlake, cellSalt, cellCounter)) blocks bytes cptr = do
	    let bits = cryptoCoerce bytes :: BITS Word64
	        len  = cryptoCoerce blocks + bits
	        p = primitiveOf g
	        block = blockSize p
	        padl = padLength p len
	        padBlocks = cryptoCoerce padl `asTypeOf` blocks
	        tBlocks = cryptoCoerce (bytes + padl)
	    unsafePad p len (cptr `movePtr` bytes)
	    if padBlocks==0
		    then do
		    	apply g (tBlocks-1) cptr
	    		cellModify cellCounter (\a -> a - cryptoCoerce padl)
	    		apply g 1 (cptr `movePtr` (tBlocks - 1))
	    	else do
	    		apply g (tBlocks-2) cptr
	    		cellModify cellCounter (\a -> a - cryptoCoerce (padl - block))
