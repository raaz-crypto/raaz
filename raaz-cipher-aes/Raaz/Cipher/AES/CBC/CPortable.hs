...
foreign import ccall unsafe
  "raaz/cipher/cportable/aes.c raazCipherAESEncryptCBC128"
  c_cbc_encrypt128  :: CryptoPtr  -- ^ expanded key
                    -> CryptoPtr  -- ^ Input
                    -> CryptoPtr  -- ^ IV
...
foreign import ccall unsafe
  "raaz/cipher/cportable/aes.c raazCipherAESEncryptCBC192"
  c_cbc_encrypt192  :: CryptoPtr  -- ^ expanded key
                    -> CryptoPtr  -- ^ Input
                    -> CryptoPtr  -- ^ IV
                    -> Int        -- ^ Number of Blocks
                    -> IO ()
...
foreign import ccall unsafe
  "raaz/cipher/cportable/aes.c raazCipherAESEncryptCBC256"
  c_cbc_encrypt256  :: CryptoPtr  -- ^ expanded key
                    -> CryptoPtr  -- ^ Input
                    -> CryptoPtr  -- ^ IV
                    -> Int        -- ^ Number of Blocks
                    -> IO ()
...
foreign import ccall unsafe
  "raaz/cipher/cportable/aes.c raazCipherAESDecryptCBC128"
  c_cbc_decrypt128  :: CryptoPtr  -- ^ expanded key
                    -> CryptoPtr  -- ^ Input
                    -> CryptoPtr  -- ^ IV
                    -> Int        -- ^ Number of Blocks
                    -> IO ()
...
foreign import ccall unsafe
  "raaz/cipher/cportable/aes.c raazCipherAESDecryptCBC192"
  c_cbc_decrypt192  :: CryptoPtr  -- ^ expanded key
                    -> CryptoPtr  -- ^ Input
                    -> CryptoPtr  -- ^ IV
                    -> Int        -- ^ Number of Blocks
                    -> IO ()
...
foreign import ccall unsafe
  "raaz/cipher/cportable/aes.c raazCipherAESDecryptCBC256"
  c_cbc_decrypt256  :: CryptoPtr  -- ^ expanded key
                    -> CryptoPtr  -- ^ Input
                    -> CryptoPtr  -- ^ IV
                    -> Int        -- ^ Number of Blocks
                    -> IO ()
...
instance Gadget (CPortable128 Encryption) where
  type PrimitiveOf (CPortable128 Encryption) = AES128 CBC Encryption
  ...
  apply (CPortable128 (ek,civ)) n cptr = withCell ek (withCell civ . doStuff)
    where
      doStuff ekptr ivptr = c_cbc_encrypt128 ekptr cptr ivptr (fromIntegral n)

instance Gadget (CPortable128 Decryption) where
  type PrimitiveOf (CPortable128 Decryption) = AES128 CBC Decryption
  ...
  apply (CPortable128 (ek,civ)) n cptr = withCell ek (withCell civ . doStuff)
    where
      doStuff ekptr ivptr = c_cbc_decrypt128 ekptr cptr ivptr (fromIntegral n)


--------------------- AES192 ---------------------------------------------------

instance Gadget (CPortable192 Encryption) where
  type PrimitiveOf (CPortable192 Encryption) = AES192 CBC Encryption
  ...
  apply (CPortable192 (ek,civ)) n cptr = withCell ek (withCell civ . doStuff)
    where
      doStuff ekptr ivptr = c_cbc_encrypt192 ekptr cptr ivptr (fromIntegral n)

instance Gadget (CPortable192 Decryption) where
  type PrimitiveOf (CPortable192 Decryption) = AES192 CBC Decryption
  ...
  apply (CPortable192 (ek,civ)) n cptr = withCell ek (withCell civ . doStuff)
    where
      doStuff ekptr ivptr = c_cbc_decrypt192 ekptr cptr ivptr (fromIntegral n)


--------------------- AES256 ---------------------------------------------------

instance Gadget (CPortable256 Encryption) where
  type PrimitiveOf (CPortable256 Encryption) = AES256 CBC Encryption
  ...
  apply (CPortable256 (ek,civ)) n cptr = withCell ek (withCell civ . doStuff)
    where
      doStuff ekptr ivptr = c_cbc_encrypt256 ekptr cptr ivptr (fromIntegral n)

instance Gadget (CPortable256 Decryption) where
  type PrimitiveOf (CPortable256 Decryption) = AES256 CBC Decryption
  ...
  apply (CPortable256 (ek,civ)) n cptr = withCell ek (withCell civ . doStuff)
    where
      doStuff ekptr ivptr = c_cbc_decrypt256 ekptr cptr ivptr (fromIntegral n)
