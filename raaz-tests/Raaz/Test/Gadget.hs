...
prop_Gadget ref' g' iv (TestData bs) = monadicIO $ do
  ...
  outg <- run $ allocaBytes bsize (with g)
  outref <- run $ allocaBytes bsize (with ref)
  assert (outg == outref)
  where
    bsize = B.length bs
    ...
    with g cptr =  do
      unsafeCopyToCryptoPtr bs cptr
      apply g (fromIntegral numBlocks) cptr
      out <- finalize g
      outbs <- create bsize copyTo
      return (out,outbs)
      where
        copyTo ptr = memcpy ptr (castPtr cptr) bsize
